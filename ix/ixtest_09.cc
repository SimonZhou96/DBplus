...
int testCase_9(const std::string &indexFileName, const Attribute &attribute) {
    ...
    for (unsigned i = 1; i <= numOfTuples; i++) {
        ...
        if (i == 410 || i == 409) {
            ...
            ixFileHandle.readIxPage(3,page);
            unsigned numberOfKeys = *(unsigned *)((char *)page + PAGE_SIZE - TYPE_PTR - 2 * sizeof(unsigned));
            auto *test = (char *)page + sizeof(unsigned);
            for (int i = 0; i < numberOfKeys; i++) {
                cout << "index " << *(float *)test;
                test += sizeof(float) + 3 * sizeof(unsigned);
            }
            cout << endl;
