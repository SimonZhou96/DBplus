...
int RBFTest_Delete(RecordBasedFileManager &rbfm) {
    ...
    assert(rc == success && "Destroying the file should not fail.");
    /** test for huge insert and delete */
    ...
    //                      &recordSize);
    //        rbfm.insertRecord(fileHandle, recordDescriptor, data, rid[i]);
    //        void *readData = malloc(PAGE_SIZE);
    //        rbfm.readRecord(fileHandle, recordDescriptor, rid[i], readData);
    ...
    //    int i = 0;
    //    while (i < 5000) {
    //        rbfm.deleteRecord(fileHandle,recordDescriptor, rid[i]);
    //        void * data = malloc(PAGE_SIZE);
    //        if (rbfm.readRecord(fileHandle,recordDescriptor,rid[i],data) == 0) perror("delete error!");
    //        free(data);
    //        i++;
    //    }
    free(record);
